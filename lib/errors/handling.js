...
function convertError(error) {
    ...
    if (error.validation) {
        ...
        return validation_1.convertValidationErrors({}, error.validation, prefix);
    }
...
function handleInternalError(error, _r, reply) {
    const boom = error.isBoom ? error : convertError(error);
    reply
