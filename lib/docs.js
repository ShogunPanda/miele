...
function printRoutes(routes) {
    ...
    routes = routes
        .filter((r) => !favo_1.get(r, 'config.hide', false) && !favo_1.get(r, 'schema.hide', false))
    ...
    const output = routes.map((route) => {
        ...
        return `﹒ \x1b[32m${method}\x1b[0m ${url} \x1b[37m${favo_1.get(route, 'config.description', '')}\x1b[0m`;
    });
